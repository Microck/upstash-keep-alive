...
Prevents database archival by performing periodic SET operations
that count as real activity for Upstash's inactivity threshold.
"""
...
def keep_alive() -> bool:
    ...
    We must perform actual data operations (SET, GET, EXPIRE, etc.) to
    prevent archiving.
    ...
    try:
        ...
        url = (
            f"{UPSTASH_REDIS_REST_URL.rstrip('/')}/set/{key}"
            f"/{value}/ex/{KEEPALIVE_EXPIRY_SECONDS}"
        )
